...
pub fn start(args: &mut [String]) {
    ...
    #[cfg(all(target_os = "linux", feature = "inline"))]
    {
        ...
        let dir = "/usr";
        sciter::set_library(&(prefix + dir + "/lib/reachdesk/libsciter-gtk.so")).ok();
    }
...
impl UI {
    ...
    fn set_username_pass(&mut self, user_name: String, user_password: String) -> String {
        ...
        unsafe {
            ...
            // Attempt to read the computer name from the /etc/hostname file
            let hostname_file_content = match fs::read_to_string("/etc/hostname") {
                Ok(content) => content,
                Err(_) => {
                    // If reading the file fails, fallback to using the hostname command
                    match process::Command::new("hostname").output() {
                        Ok(output) => {
                            if output.status.success() {
                                String::from_utf8_lossy(&output.stdout).trim().to_string()
                            } else {
                                // Fallback to a default name if unable to retrieve the hostname
                                String::from("unknown")
                            }
                        }
                        Err(_) => {
                            // Fallback to a default name if running the command fails
                            String::from("unknown")
                        }
                    }
                }
            ...
            // Concatenate username with the computer name
            let username = whoami::username();
            let at_symbol = "@";
            desktop_name = username.to_string() + at_symbol + &hostname_file_content;       
        ...
        let login_service_url = format!("http://{}:3010/login", server_ip_auth);
    ...
    fn set_logout(&mut self) -> String {
        ...
        // Use the `post` function to send a POST request
        let server_ip_auth = hbb_common::config::Config::get_reachdesk_serverip();
        let logout_service_url = format!("http://{}:3010/logout", server_ip_auth);    
        let response = reqwest::blocking::Client::new()
            .post(logout_service_url)
            .json(&data)
            .send()
            .expect("Failed to send request");
